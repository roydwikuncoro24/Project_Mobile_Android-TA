...
public class AdminProfil extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dialogtoko = new SpotsDialog(AdminProfil.this, "Loading...");
    ...
    public void getProfile(){
        ...
        Call<GetProfilResponse> call = api.getAdmin(((AdminModelData) tinyDB.getObject("admin_login", AdminModelData.class)).getIdadmin());
        ...
        call.enqueue(new Callback<GetProfilResponse>() {
            ...
            public void onResponse(Call<GetProfilResponse> call, retrofit2.Response<GetProfilResponse> response) {
                ...
                if (response.body().getStatus_code()==1) {
                    nama.setText(response.body().getResult_admin().get(0).getNama());
                    telp.setText(response.body().getResult_admin().get(0).getNotelp());
                    alamat.setText(response.body().getResult_admin().get(0).getAlamat());
                    mail.setText(response.body().getResult_admin().get(0).getEmail());
                    dialogtoko.dismiss();
                }
            }
            @Override
            public void onFailure(Call<GetProfilResponse> call, Throwable t) {
                if (t instanceof SocketTimeoutException) {
                    dialogtoko.dismiss();
                    Toast.makeText(AdminProfil.this, "Harap periksa koneksi internet", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public void updateProfile(){
        dialogtoko.show();
        APIService api = RetrofitHelper.getClient().create(APIService.class);
        Call<InsertResponse> call = api.updateProfilAdmin(((AdminModelData) tinyDB.getObject("admin_login", AdminModelData.class)).getIdadmin(),
        ...
        call.enqueue(new Callback<InsertResponse>() {
            ...
            public void onResponse(Call<InsertResponse> call, Response<InsertResponse> response) {
                ...
                if (response.body().getStatus_code().equals("1")) {
                    dialogtoko.dismiss();
                    Toast.makeText(AdminProfil.this, "Update berhasil", Toast.LENGTH_SHORT).show();
                    AdminModelData admin = ((AdminModelData) tinyDB.getObject("admin_login", AdminModelData.class));
                    admin.setEmail(mail.getText().toString());
                    admin.setNotelp(telp.getText().toString());
                    admin.setAlamat(alamat.getText().toString());
                    tinyDB.putObject("admin_login",admin);
                    getProfile();
                }else{
                    dialogtoko.dismiss();
                    Toast.makeText(AdminProfil.this, "Update gagal", Toast.LENGTH_SHORT).show();
                }
            }
            @Override
